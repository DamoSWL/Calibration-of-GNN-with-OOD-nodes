...
def load_npz_data(dataset_str, seed, task_type): 
    ...
    degree = None
    learning_type = None
    return adj, features, labels, idx_train, idx_val, idx_test, degree, learning_type, test_mask
...
def data_loader_OOD(dataset, data_path=datadir, normalization="AugNormAdj", porting_to_torch=True, task_type = "full"):
    if dataset in ["cora", "citeseer", "pubmed"]:
        (adj,
         ...
         idx_test_id,
         test_mask_id,
         test_mask_ood) = load_citation_ood(dataset, normalization, porting_to_torch, data_path, task_type)
        # print("citation", labels)
        train_adj = adj
        train_features = features
        return adj, train_adj, features, train_features, labels, idx_train, idx_val, idx_test, degree, learning_type, idx_test_ood, idx_test_id, test_mask_id, test_mask_ood
        ...
        (adj,
         ...
         idx_test_id,
         test_mask_id,
         test_mask_ood) = load_npz_data_ood_train(dataset, SEED) 
        train_adj = adj
        train_features = features
        return adj, train_adj, features, train_features, labels, idx_train, idx_val, idx_test, degree, learning_type, idx_test_ood, idx_test_id, test_mask_id, test_mask_ood
...
def load_citation_ood(dataset_str="citeseer", normalization="AugNormAdj", porting_to_torch=True, data_path=datadir, task_type="full"):
    ...
    if dataset_str == 'cora':
        ...
        val_mask[mask] = False
        test_mask_id[mask] = False
        labels[mask] = -1

        mask = category <= 3
        test_mask_ood[mask] = False
    ...
    return adj, features, labels, idx_train, idx_val, idx_test, degree, learning_type, idx_test_ood, idx_test_id, test_mask_id, test_mask_ood
